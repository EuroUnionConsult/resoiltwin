...
|---|---|---|---|
| 1 | muta numa **copia**, nunca na arvore real | um ficheiro mutado durante oito segundos e um falso alarme a espera de acontecer para quem estiver a rever a arvore | `test_guarda_1_a_ronda_nao_toca_na_arvore_real`, `..._recusa_uma_arvore_de_trabalho_dentro_da_arvore_real` |
| 2 | **sabota a copia** e exige que a suite caia | um pacote instalado em modo editavel faz importar a arvore REAL, e entao todos os mutantes "sobrevivem" por uma razao que nada tem a ver com os testes | `test_guarda_2_aborta_quando_a_sentinela_nao_e_importada` |
| 3 | exige **base verde** (e que recolha testes) | a mentira da manha | `test_guarda_3_aborta_com_a_base_vermelha`, `..._quando_a_base_nao_recolhe_testes` |
...
| 7 | **`timeout`** no subprocesso | um mutante matou por bloqueio e nao por falha | `test_guarda_7_um_mutante_que_bloqueia_nao_conta_como_morto` |
...
| 12 | recusa um **mutante nulo** | um mutante que nao muda nada sobrevive sempre, e le-se como teste em falta | `test_guarda_12_recusa_um_mutante_que_nao_muda_nada` |
