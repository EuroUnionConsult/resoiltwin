...
from resoiltwin.attention import ATTENTION_REASON
...
def list_jobs(
    ...
    O `attention` vem em todas as linhas e nao so quando se filtra por ele: uma
    listagem que so dissesse o veredicto a quem ja o tivesse pedido obrigava a
    saber da pergunta para obter a resposta.
    ...
    """
    consulta = (
        select(IngestionJob, ATTENTION_REASON)
    ...
    return [
        IngestionJobStatusRead.a_partir_de(job, atencao)
        for job, atencao in session.execute(consulta)
    ]
...
def read_job(job_id: uuid.UUID, session: Session = Depends(get_session)):
    ...
    E o mesmo `attention` calculado pela mesma expressao: quem chega aqui vindo
    de um `POST .../sync` fica a saber, sem ter de ir buscar a listagem, se
    aquela linha e das que precisam de atencao.
    """
    linha = session.execute(
        select(IngestionJob, ATTENTION_REASON).where(IngestionJob.id == job_id)
    ).one_or_none()
    if linha is None:
